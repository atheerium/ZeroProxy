...
async fn mitm_tls_e2e_round_trip() {
    ...
    // ── 2. Generate CA ──
    let ca = zeroproxy::core::mitm::cert::generate_ca().expect("CA generation");
    ...
    // ── 4. Start MITM proxy ──
    let mut handle = zeroproxy::core::mitm::server::start_mitm_proxy(
        zeroproxy::core::mitm::server::MitmProxyConfig {
