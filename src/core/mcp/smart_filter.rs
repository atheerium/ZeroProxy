...
pub fn smart_filter_text(text: &str) -> Option<String> {
    ...
    let final_text = if collapsed.len() > MAX_TEXT_CHARS {
        ...
        format!(
            "{head}\n\n... [truncated {} chars by zeroproxy bridge. Page is large; ask user to scroll/navigate to a specific section, or click an element with the refs shown above]",
            text.len() - head.len()
        )
    } else {
        collapsed
...
fn collapse_repeated(text: &str) -> String {
    ...
    while i < lines.len() {
        ...
        if group_len >= COLLAPSE_THRESHOLD {
            ...
            out.push(format!(
                "{indent}... [{omitted} similar \"{role}\" items omitted by zeroproxy bridge]"
            ));
...
mod tests {
    ...
    fn truncates_at_max_chars() {
        ...
        assert!(out.contains("zeroproxy bridge"));
    }
