//! `zeroproxy provider oauth *` — wrap the OAuth/device-code/import endpoints.
//!
...
async fn run_start(
    ...
    match rt.get_json(&path).await {
        Ok(payload) => {
            if ctx.is_robot() {
                emit_robot("zeroproxy.v1.oauth.start", payload)?;
            } else {
                let url = payload
                    .get("url")
...
async fn run_poll(
    ...
    match rt.post_json(&path, &body).await {
        Ok(payload) => {
            if ctx.is_robot() {
                emit_robot("zeroproxy.v1.oauth.poll", payload)?;
            } else {
...
async fn run_status(rt: &Runtime, ctx: OutputCtx, provider: &str) -> anyhow::Result<i32> {
    ...
    match rt.get_json(&path).await {
        Ok(payload) => {
            if ctx.is_robot() {
                emit_robot("zeroproxy.v1.oauth.status", payload)?;
            } else {
                humanln(
                    ctx,
...
async fn run_refresh(
    ...
    match rt.post_json(&path, &body).await {
        Ok(payload) => {
            if ctx.is_robot() {
                emit_robot("zeroproxy.v1.oauth.refresh", payload)?;
            } else {
                let status = payload
                    .get("status")
                    .and_then(Value::as_str)
...
async fn run_import_kiro(rt: &Runtime, ctx: OutputCtx, auto: bool) -> anyhow::Result<i32> {
    if auto {
        match rt.get_json("/api/oauth/kiro/auto-import").await {
            Ok(payload) => {
                if ctx.is_robot() {
                    emit_robot("zeroproxy.v1.oauth.import_kiro", payload)?;
                } else {
                    humanln(
                        ctx,
                        format!(
...
async fn run_iflow_cookie(rt: &Runtime, ctx: OutputCtx, source: &str) -> anyhow::Result<i32> {
    ...
    match rt.post_json("/api/oauth/iflow/cookie", &body).await {
        Ok(payload) => {
            if ctx.is_robot() {
                emit_robot("zeroproxy.v1.oauth.iflow_cookie", payload)?;
            } else {
...
async fn run_gitlab_pat(rt: &Runtime, ctx: OutputCtx, source: &str) -> anyhow::Result<i32> {
    ...
    match rt.post_json("/api/oauth/gitlab/pat", &body).await {
        Ok(payload) => {
            if ctx.is_robot() {
                emit_robot("zeroproxy.v1.oauth.gitlab_pat", payload)?;
            } else {
