...
pub struct Cli {
    ...
    /// Emit a stable JSON envelope (`zeroproxy.v1.*`) to stdout for agents.
...
pub enum ServerCmd {
    ...
    Start {
        ...
        /// and SKILL.md show `zeroproxy server start --detach --no-open`.
        #[arg(long)]
        no_open: bool,
    },
...
#[derive(Debug, Clone, Subcommand)]
pub enum AuthCmd {
    /// Save credentials for a remote zeroproxy server as a profile.
    Login {
        /// Base URL of the remote server, e.g. https://op.example.com:4623
        #[arg(long)]
        url: String,
        /// API key generated by `zeroproxy server init` on the remote server.
        #[arg(long)]
        api_key: String,
...
pub enum SchemaCmd {
    ...
    /// `zeroproxy.v1.*` envelopes are declared **stable** (additive-only).
    Stability,
}
        ...
        KeyCmd::Add {
            ...
            let secret = if auto {
                ...
                let exit = output::emit_error(
                    ctx,
                    "usage",
                    "missing key secret. Pass a value positionally (`zeroproxy key add <NAME> <SECRET>`) or use `--auto` to generate one.",
                )?;
                std::process::exit(exit);
            } else {
                key
            ...
            if ctx.is_robot() {
                ...
                output::emit_robot("zeroproxy.v1.key.add", payload)?;
            } else if json {
...
pub async fn run_pool(cmd: PoolCmd, db: &Db, ctx: output::OutputCtx) -> anyhow::Result<()> {
    match cmd {
        ...
        PoolCmd::Create {
            ...
            if ctx.is_robot() {
                output::emit_robot("zeroproxy.v1.pool.create", serde_json::to_value(&new_pool)?)?;
            } else if json {
...
async fn run_route(
    ...
    if let (Some(model_str), None) = (&model, &combo) {
        ...
        eprintln!("Usage: zeroproxy route --model cc/claude-opus-5 --prompt 'hello'");
        eprintln!("   or: zeroproxy route --combo default --prompt 'hello'");
        std::process::exit(1);
    }
}
