//! `zeroproxy tunnel *` — runtime tunnel commands (PLAN v3 mục 4.11).
//!
    ...
    match result {
        Ok(payload) => {
            let schema = if enable {
                "zeroproxy.v1.tunnel.enable"
            } else {
                "zeroproxy.v1.tunnel.disable"
            };
            if ctx.is_robot() {
...
async fn simple(rt: &Runtime, ctx: OutputCtx, path: &str, action: &str) -> anyhow::Result<i32> {
    match rt.post_empty(path).await {
        Ok(payload) => {
            if ctx.is_robot() {
                emit_robot(
                    &format!("zeroproxy.v1.tunnel.tailscale.{}", action),
                    payload,
                )?;
            } else {
                let msg = payload
...
async fn run_check(rt: &Runtime, ctx: OutputCtx) -> anyhow::Result<i32> {
    match rt.get_json("/api/tunnel/tailscale-check").await {
        Ok(payload) => {
            if ctx.is_robot() {
                emit_robot("zeroproxy.v1.tunnel.tailscale.check", payload)?;
            } else {
