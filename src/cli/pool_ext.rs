//! `zeroproxy pool *` — extended proxy pool commands (M3).
//!
...
async fn run_get(db: &Db, ctx: OutputCtx, name: &str) -> anyhow::Result<()> {
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.pool.get", serde_json::to_value(&pool)?)?;
    } else {
...
async fn run_edit(
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.pool.edit", serde_json::to_value(&pool)?)?;
    } else {
...
async fn run_set_active(db: &Db, ctx: OutputCtx, name: &str, active: bool) -> anyhow::Result<()> {
    ...
    let schema = if active {
        "zeroproxy.v1.pool.enable"
    } else {
        "zeroproxy.v1.pool.disable"
    };
    if ctx.is_robot() {
...
async fn run_test(db: &Db, ctx: OutputCtx, name: &str, target: &str) -> anyhow::Result<()> {
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.pool.test", payload)?;
    } else if valid {
...
async fn run_stats(db: &Db, ctx: OutputCtx, name: &str) -> anyhow::Result<()> {
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.pool.stats", payload)?;
    } else {
