//! `zeroproxy logs *` — tail/export/clear the in-memory log buffer.
//!
...
async fn run_export(rt: &Runtime, ctx: OutputCtx, out: &str) -> anyhow::Result<i32> {
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.log.export", snapshot)?;
        return Ok(0);
    }
    let logs = snapshot
        .get("logs")
...
async fn run_clear(rt: &Runtime, ctx: OutputCtx) -> anyhow::Result<i32> {
    match rt.post_empty("/api/observability/clear").await {
        Ok(payload) => {
            if ctx.is_robot() {
                emit_robot("zeroproxy.v1.log.clear", payload)?;
            } else {
...
async fn run_stats(rt: &Runtime, ctx: OutputCtx) -> anyhow::Result<i32> {
    match rt.get_json("/api/observability/stats").await {
        Ok(payload) => {
            if ctx.is_robot() {
                emit_robot("zeroproxy.v1.log.stats", payload)?;
            } else {
                humanln(
                    ctx,
                    format!(
