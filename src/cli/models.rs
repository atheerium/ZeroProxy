//! `zeroproxy models *` — top-level model registry view.
//!
...
async fn run_info(db: &Db, ctx: OutputCtx, model: &str) -> anyhow::Result<()> {
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.models.info", payload)?;
    } else {
...
async fn run_test(db: &Db, ctx: OutputCtx, model: &str) -> anyhow::Result<()> {
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.models.test", payload)?;
    } else if valid {
        humanln(
            ctx,
...
async fn run_pricing(db: &Db, ctx: OutputCtx, model: Option<&str>) -> anyhow::Result<()> {
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.models.pricing", pricing)?;
    } else {
