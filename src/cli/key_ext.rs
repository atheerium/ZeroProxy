//! `zeroproxy key *` — extended API key lifecycle commands (M3).
//!
...
async fn run_get(db: &Db, ctx: OutputCtx, id_or_name: &str) -> anyhow::Result<()> {
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.key.get", payload)?;
    } else {
...
async fn run_rotate(db: &Db, ctx: OutputCtx, id_or_name: &str) -> anyhow::Result<()> {
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.key.rotate", payload)?;
    } else {
...
async fn run_set_active(
    ...
    let schema = if active {
        "zeroproxy.v1.key.enable"
    } else {
        "zeroproxy.v1.key.disable"
    };
    if ctx.is_robot() {
