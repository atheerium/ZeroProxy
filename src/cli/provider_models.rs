//! `zeroproxy provider models *` — model registry per provider.
//!
...
async fn run_list(db: &Db, ctx: OutputCtx, provider: &str) -> anyhow::Result<()> {
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.provider-models.list", payload)?;
    } else {
...
async fn run_test(
    ...
    if ctx.is_robot() {
        emit_robot("zeroproxy.v1.provider-models.test", payload)?;
    } else if valid {
...
async fn run_alias(db: &Db, ctx: OutputCtx, cmd: AliasCmd) -> anyhow::Result<()> {
    match cmd {
        AliasCmd::List => {
            ...
            if ctx.is_robot() {
                emit_robot(
                    "zeroproxy.v1.provider-models.alias.list",
        ...
        AliasCmd::Set {
            ...
            if ctx.is_robot() {
                emit_robot("zeroproxy.v1.provider-models.alias.set", payload)?;
            } else {
        ...
        AliasCmd::Unset { alias } => {
            ...
            if !existed {
                if ctx.is_robot() {
                    emit_robot(
                        "zeroproxy.v1.provider-models.alias.unset",
...
async fn run_set_disabled(
    ...
    let schema = if disable {
        "zeroproxy.v1.provider-models.disable"
    } else {
        "zeroproxy.v1.provider-models.enable"
    };
    if ctx.is_robot() {
        emit_robot(
            schema,
...
async fn run_custom(db: &Db, ctx: OutputCtx, cmd: CustomCmd) -> anyhow::Result<()> {
    match cmd {
        CustomCmd::List => {
            ...
            if ctx.is_robot() {
                emit_robot(
                    "zeroproxy.v1.provider-models.custom.list",
        ...
        CustomCmd::Add {
            ...
            if ctx.is_robot() {
                emit_robot(
                    "zeroproxy.v1.provider-models.custom.add",
        ...
        CustomCmd::Remove { provider, model } => {
            ...
            if !existed {
                if ctx.is_robot() {
                    emit_robot(
                        "zeroproxy.v1.provider-models.custom.remove",
                        json!({
                            "provider": provider,
                            "model": model,
                            "removed": false,
