#!/usr/bin/env bash
# install.sh — one-shot installer for zeroproxy
#
# Usage:
...
#   --no-skill             Skip installing the agent skill into ~/.agents/skills/zeroproxy/SKILL.md
...
#   zeroproxy-vX.Y.Z-<suffix>.tar.gz
detect_platform() {
    local os arch
    case "$(uname -s)" in
        Linux*)  os="linux";;
...
# ════════════════════════════════════════════════════════════════════════════
# Agent skill install — drops SKILL.md into ~/.agents/skills/zeroproxy/ so
# agents that auto-discover .agents/skills/ (Devin, Claude Code, ...) can
# install + operate zeroproxy on the user's behalf.
#
# Idempotent: if the destination file already exists and was NOT written by
# this installer (i.e. doesn't start with our "name: zeroproxy" frontmatter),
...
install_completions() {
    ...
    if curl -fsSL --connect-timeout 10 --max-time 15 -o "$bash_dir/zeroproxy" "$completions_url/zeroproxy.bash" 2>/dev/null; then
        log_success "bash completions → $bash_dir/zeroproxy"
    else
    ...
    if curl -fsSL --connect-timeout 10 --max-time 15 -o "$zsh_dir/_zeroproxy" "$completions_url/zeroproxy.zsh" 2>/dev/null; then
        log_success "zsh completions → $zsh_dir/_zeroproxy"
    else
    ...
    if curl -fsSL --connect-timeout 10 --max-time 15 -o "$fish_dir/zeroproxy.fish" "$completions_url/zeroproxy.fish" 2>/dev/null; then
        log_success "fish completions → $fish_dir/zeroproxy.fish"
    else
